...
parser.add_argument("--ckpt-path", default=".\model_weights_best-50.chpt")
parser.add_argument("--result", default=".\\results\\test.json")
...
def test():
    ...
    model = resnet_model.get_model(True).to(device)
    ...
    for i, (imgs, masks, labels, img_ids) in enumerate(dataloaders):
        ...
        with torch.no_grad():
            ...
            sample_to_result[img_ids[0].item()] = {'z_mean': z_means.item(), 'z_std': z_stds.item(), 'x_std': x_stds.item(), 'y_std': y_stds.item()}
    ...
    plotting_script.plot_single_dist({'Ground Truth Height Distribution': all_height_labels, 'Prediction Height Distribution': all_height_predictions},
                                     'Height (cm)', 'Number of Buildings', 'Prediction and Label Distribution', ['Ground Truth Height Distribution', 'Prediction Height Distribution'])
    plotting_script.plot_box_chart([all_height_error, short_errors, tall_errors], 'Absolute Error', 'Value(cm)', 'Absolute Error Distribution',
